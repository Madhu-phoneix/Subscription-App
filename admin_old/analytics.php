...
$shop_timezone = $mainobj->shop_timezone;
$current_date = strtok($mainobj->getCurrentUtcTime() ," ");  //get utc current date
$shop_current_date = $mainobj->getShopTimezoneDateFormat($current_date,$shop_timezone); //convert utc date to shopify time format
...
if(isset($_GET['start_date']) && isset($_GET['last_date'])){
    ...
    if($start_date != '' && $last_date == ''){
      ...
      $last_date = strtok($mainobj->getCurrentUtcTime() ," ");
      $start_date = date('Y-m-d', strtotime('-7 days', strtotime($last_date)));
    }
}
...
$get_shop_analytics_data = $mainobj->customQuery("SELECT subscriptionPlanGroups.store_id, (SELECT COUNT(*) FROM subscriptionPlanGroups as g WHERE g.store_id='$mainobj->store_id' ) as total_subscription_group,(SELECT COUNT(*) FROM subscriptionOrderContract as s WHERE s.contract_status = 'A' and s.store_id='$mainobj->store_id') as total_active_subscription_contract,(SELECT COUNT(*) FROM subscriptionOrderContract as s WHERE s.contract_status = 'P' and s.store_id='$mainobj->store_id') as total_pause_subscription_contract FROM subscriptionPlanGroups ,subscriptionOrderContract where subscriptionPlanGroups.store_id = '$mainobj->store_id' GROUP BY subscriptionPlanGroups.store_id");

$selling_plan_count_data = $mainobj->customQuery("SELECT selling_plan_id, COUNT(*) AS total_orders FROM subscriptionOrderContract  where subscriptionOrderContract.store_id = '$mainobj->store_id' GROUP BY selling_plan_id  order by COUNT(*) desc, id desc limit 7");
...
if(!empty($selling_plan_count_data)){
  ...
  $get_selling_and_group_name = $mainobj->customQuery("SELECT subscriptionPlanGroupsDetails.plan_name,subscriptionPlanGroupsDetails.selling_plan_id, subscriptionPlanGroups.plan_name as selling_plan_group_name FROM subscriptionPlanGroupsDetails INNER JOIN subscriptionPlanGroups ON subscriptionPlanGroups.subscription_plangroup_id = subscriptionPlanGroupsDetails.subscription_plan_group_id WHERE subscriptionPlanGroupsDetails.selling_plan_id IN ($selling_plan_id)");
}

$date_range_sale_from_billingAttempts = $mainobj->customQuery("SELECT BA.created_at, BA.order_total,O.order_currency,O.contract_id FROM billingAttempts AS BA, subscriptionOrderContract as O where BA.store_id = '$mainobj->store_id' and BA.status = 'Success' and BA.contract_id = O.contract_id  and BA.created_at BETWEEN '$start_date' AND '$query_last_date'");
$total_date_price_array = $date_range_sale_from_billingAttempts;
// echo '<pre>';
// print_r($total_date_price_array);

//total subscription orders
$get_total_subscription_orders = $mainobj->customQuery("SELECT COUNT(distinct(order_no)) as total_contract_orders from subscriptionOrderContract where  store_id = '$mainobj->store_id'");

$get_billing_attempt_orders = $mainobj->customQuery("SELECT COUNT(distinct(order_no)) as total_billing_attempt_orders from billingAttempts where  store_id = '$mainobj->store_id' ");
$total_orders = $get_total_subscription_orders[0]['total_contract_orders'] + $get_billing_attempt_orders[0]['total_billing_attempt_orders'];
      ...
      <div class="Polaris-FormLayout__Item sd_productPageSetting">
          <div class="Polaris-Card">
            ...
            <div class="Polaris-Card__Header">
              ...
              <a href="<?php echo $mainobj->SHOPIFY_DOMAIN_URL; ?>/admin/plans/subscription_group.php?shop=<?php echo $mainobj->store; ?>" target= "_blank">
                <span class="Polaris-TextStyle--variationSubdued"><?php if(!empty($get_shop_analytics_data[0]['total_subscription_group'])){ echo $get_shop_analytics_data[0]['total_subscription_group']; }else{ echo '0'; } ?></span>
                </a>
              </div>
        </div>
          </div>
        ...
        <div class="Polaris-FormLayout__Item sd_productPageSetting">
          <div class="Polaris-Card">
            ...
            <a href="<?php echo $mainobj->SHOPIFY_DOMAIN_URL; ?>/admin/subscription/subscriptions.php?shop=<?php echo $mainobj->store; ?>&search_contract=active" target= "_blank">
              <div class="Polaris-Card__Section sd_analytic_badge "><span class="Polaris-TextStyle--variationSubdued"><?php if(!empty($get_shop_analytics_data[0]['total_subscription_group'])){  echo $get_shop_analytics_data[0]['total_active_subscription_contract']; }else{ echo '0'; }?></span>
              </div>
            </a>
              </div>
          </div>
        ...
        <div class="Polaris-FormLayout__Item sd_productPageSetting">
          <div class="Polaris-Card">
            ...
            <a href="<?php echo $mainobj->SHOPIFY_DOMAIN_URL; ?>/admin/subscription/subscriptions.php?shop=<?php echo $mainobj->store; ?>&search_contract=pause" target= "_blank">
            <div class="Polaris-Card__Section sd_analytic_badge "><span class="Polaris-TextStyle--variationSubdued"><?php if(!empty($get_shop_analytics_data[0]['total_subscription_group'])){  echo $get_shop_analytics_data[0]['total_pause_subscription_contract']; }else{ echo '0'; }?></span>
            </div>
            </a>
              </div>
          </div>
        ...
        <div class="Polaris-FormLayout__Item sd_productPageSetting">
          <div class="Polaris-Card">
            ...
            <a href="https://<?php echo $mainobj->store; ?>/admin/orders?inContextTimeframe=none&query=sd_recurring_subscription_order" target= "_blank">
            <div class="Polaris-Card__Section sd_analytic_badge"><span class="Polaris-TextStyle--variationSubdued"><?php echo $get_billing_attempt_orders[0]['total_billing_attempt_orders']; ?></span>
            </div>
            </a>
              </div>
          </div>
          ...
          <?php  if(!empty($get_selling_and_group_name)){ ?>
            ...
            <table class="Polaris-IndexTable__Table most_purchased_selling">
              ...
              <tbody>
                    ...
                    ?>
                 <tr class="Polaris-DataTable__TableRow Polaris-DataTable--hoverable"><td class="Polaris-DataTable__Cell Polaris-DataTable__Cell--verticalAlignTop Polaris-DataTable__Cell--numeric"><a href="<?php echo $mainobj->SHOPIFY_DOMAIN_URL; ?>/admin/plans/subscription_group.php?shop=<?php echo $mainobj->store; ?>&search_subscription_group_name=<?php echo $value['selling_plan_group_name'];  ?>" target= "_blank"><?php echo $value['selling_plan_group_name']; ?></td>
